...
class EndToEndCreateEdit {
    ...
    fun setup() {
        ...
        mockUiState2 =
            MutableStateFlow(
                InventoryUIState(listOf(item2), "", items, emptyList(), emptyList(), emptyList(), emptyList())
            )
        mockUiState3 =
            MutableStateFlow(
                InventoryUIState(listOf(item3), "", items, emptyList(), emptyList(), emptyList(), emptyList())
            )

        mockItemUiState = MutableStateFlow(
            ItemUIState(Item("", Category("", ""), "", "", Visibility.PUBLIC, 1, Location(""))))

        mockItemUiState2 = MutableStateFlow(
            ItemUIState(item2))

        mockItemUiState3 = MutableStateFlow(
            ItemUIState(item3))
    ...
    fun testB_InventoryEmptyToCreate(){
        ...
        composeTestRule.setContent { InventoryScreen(
            inventoryViewModel = mockInventoryViewModel,
            navigationActions = mockNavActions,
    ...
    fun testC_CreateItem(){
        every { mockInventoryViewModel.uiState} returns mockUiState
        every { mockItemViewModel.uiState } returns mockItemUiState
        composeTestRule.setContent { InventoryCreateOrEditItem(
            itemViewModel = mockItemViewModel,
            navigationActions = mockNavActions,
            mode = "create"
        )}
    ...
    fun testE_goToViewItem(){
        every { mockInventoryViewModel.uiState} returns mockUiState2
        every { mockItemViewModel.uiState } returns mockItemUiState2
        composeTestRule.setContent { InventoryViewItem(
            navigationActions = mockNavActions,
