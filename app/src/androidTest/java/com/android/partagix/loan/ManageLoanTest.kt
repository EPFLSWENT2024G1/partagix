...
class ManageLoanTest : TestCase(kaspressoBuilder = Kaspresso.Builder.withComposeSupport())  {
    ...
    fun testSetup() {
        ...
        val loan = listOf(Loan("2", "3", "1", Date(), Date(),
            "Borrowing requests", "Pending", "",
            "", LoanState.PENDING))
        val boolean = listOf(true)
        nonEmptyMockUiState =
            MutableStateFlow(
                ManagerUIState(items, "", users, loan, boolean)
            )
    ...
    fun noItemBoxIsDisplayed() = run {
        ...
        composeTestRule.setContent {
            ManageLoanRequest(mockManageViewModel, mockNavActions,)
        }

        ComposeScreen.onComposeScreen<ManageLoanScreen>(composeTestRule) { noItemBox { assertIsDisplayed() } }
    }
    ...
    fun bottomNavBarIsDisplayed() = run {
        ...
        ComposeScreen.onComposeScreen<ManageLoanScreen>(composeTestRule) { bottomNavBar { assertIsDisplayed() } }
    }
