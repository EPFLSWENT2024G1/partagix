...
class ManageLoanViewModel (db : Database = Database()) : ViewModel() {
    ...
    fun getInventory() {
        ...
        viewModelScope.launch {
            if (user == null) {
                database.getItems { fetchedList = it }
                database.getLoans { it.filter {loan -> /*loan.idOwner == user.uid &&*/                        loan.state == LoanState.PENDING }
                    .forEach { loan ->
                        fetchedList.filter { it.id == loan.idItem }.forEach {
                ...
                database.getItems { list ->
                    database.getLoans {
                        ...
                        it.filter { loan -> loan.state == LoanState.PENDING }
                            .forEach { loan ->
                                println(loan)
                                list.filter { it.id == loan.idItem }.forEach {
                                    database.getUser(loan.idOwner) { user -> updateUsers(user) }
                                    updateItems(it)
                                }
                                updateExpandedReset();
